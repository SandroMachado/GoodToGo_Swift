...
let SQLITE_PREPARE_ERROR : CInt = INT_MAX
...
class SQLiteDB {
	...
	class func sharedInstance() -> SQLiteDB! {
		dispatch_once(&Static.token) {
            Static.instance = self.init(gid:"", dbFileName:"")
		}
		return Static.instance!
	}
	
	class func sharedInstance(gid:String, dbFileName:String) -> SQLiteDB! {
		dispatch_once(&Static.token) {
            Static.instance = self.init(gid:gid, dbFileName:dbFileName)
		}
		return Static.instance!
	}
 
    required init(gid:String, dbFileName:String) {
		assert(Static.instance == nil, "Singleton already initialized!")
        assert(dbFileName != "", "Invalid dbFileName")
        assert(dbFileName != "", "Invalid gid")
		...
		let dbName:String = String.fromCString(dbFileName)!
		var docDir = ""
		...
		if !(fm.fileExistsAtPath(path)) {
            ...
            DLog("Database [\(dbName)] will be created")
			...
			do {
				...
				DLogError("SQLiteDB - failed to copy writable version of DB! \(error.localizedDescription)")
				return
			}
		}
		// Open the DB
		...
		if error != SQLITE_OK {
			...
			DLogError("SQLiteDB - failed to open DB!")
			sqlite3_close(db)
		}
	...
	private func closeDatabase() {
		if db != nil {
			...
			if execute(sql) != SQLITE_OK {
				DLogWarning("SQLiteDB - Error cleaning DB")
			}
			sqlite3_close(db)
		}
	}
	...
	private func prepare(sql:String, params:[AnyObject]?)->COpaquePointer {
		...
		if result != SQLITE_OK {
			...
			if let error = String.fromCString(sqlite3_errmsg(self.db)) {
				let msg = "SQLiteDB - failed to prepare query: \(sql)\nError: \(error)"
				DLogError(msg)
			}
			return nil
		}
...
//				println("Binding: \(params![ndx-1]) at Index: \(ndx)")
	...
	private func execute(stmt:COpaquePointer, sql:String)->CInt {
        ...
        if(sql.hasPrefix("UPDATE") && !sql.containsString("WHERE")) {
            DLogError("Unsafe query was ignored : \(sql)")
            return -1
        }
